...
def create_directional_positioning_showcase(df: pd.DataFrame, output_dir: str):
    ...
    # Use all-green palette when all values are non-negative, to avoid implying
    # negative performance with warm colors. Otherwise use a diverging palette.
    values = direction_performance.values
    heatmap_cmap = 'Greens' if float(np.nanmin(values)) >= 0 else 'RdYlGn'
    im = ax2.imshow(values, cmap=heatmap_cmap, aspect='auto')
